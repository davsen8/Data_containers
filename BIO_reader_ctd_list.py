...
#                        self.hdrdict["HISTORY_HEADER"][self.hdrdict["HISTORY_HEADER"]["COUNT"]] = OrderedDict()
                        self.hdrdict["HISTORY_HEADER"][self.hdrdict["HISTORY_HEADER"]["COUNT"]] = []  #list
                        line = line.rstrip()
                        

                        self.hdrdict["HISTORY_HEADER"][self.hdrdict["HISTORY_HEADER"]["COUNT"]].append(line.lstrip())
                        n=1
                        line = fp.readline()
                        line = line.rstrip()
                        while line[-1:] ==',':
    ...
    def write_JSON(self):
        with open('CTD_BCD2016666_001_01_DN_test_list.json', 'w') as fp:
